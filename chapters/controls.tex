...
Two parts to selecting gains:
\begin{itemize}
\item Find critically damped gains,
\item have angle errors converge before distance error.
\end{itemize}
The first part is done when $\zeta=\frac{\lambda_1(A)}{\lambda_2(A)}=1$ and the second part is done when $\sigma>\gamma$. This is all looking at the gains $h$, $k$ and $\gamma$ as well as the eigenvalues of the matrix $A$ below. To establish the first item, $\zeta=1$, it is necessary to have $\lambda_1(A) = \lambda_2(A) = \lambda$. For this to happen we need
\begin{align*}
&\left[\begin{array}{c c} -k-\lambda & -h\gamma \\ \gamma & 0\end{array}\right] = 0 \\
...
Using this a gain $\gamma$ can be selected first to limit the maximum linear velocity using the equation $u_{\text{max}}=\gamma e\cos(\alpha=0)=\gamma e$. For example, a distance error of $10m$ and $\gamma=0.2$ gives $u_{\text{max}}=2\tfrac{m}{s}$. Then $h$ can be selected to be small, for example $h=1.1$. Given those two variables a gain $k$ that satisfies the properties of a critically damped system for $A$ is found using $k=2\gamma\sqrt{h}=0.42$. Using these values the eigenvalues of $A$ are $\lambda_1(A) = \lambda_2(A) = -0.21$ meaning that the errors $\alpha$ and $\theta$ will decrease at the same rate. Also, since $\sigma = 0.21 > 0.2 = \gamma$ the angle errors will decrease at a faster rate than the distance error which will cause the robot to curve into the goal point with the goal heading already achieved and give a nice, smooth trajectory to the desired pose.
...
c = \frac{\omega}{u}
\end{align*}
which leads to exponentially stable convergence if and only if $\sigma > \gamma$ so that $u\neq0$ before $\omega\to0$.

Another property of the eigenvalues of $A$ involves the damping of the angle errors, or how much each angle $\alpha$ and $\theta$ will be corrected for and if one is corrected for more than the other. The damping is determined by looking at the ratio of the eigenvalues
\begin{align*}
\zeta = \frac{\lambda_1(A)}{\lambda_2(A)}.
\end{align*}
If $\zeta=1$ then $A$ is critically damped, if $\zeta>1$ then both eigenvalues are real and $A$ is overdamped and if $\zeta<1$ then the eigenvalues are a pair of complex numbers and the $A$ is underdamped.
...
\end{align*}
showing that $\sigma=1.1>\gamma=0.3$ and leading to exponentially stable convergence of the angles $\alpha$ and $\theta$ followed by exponentially stable convergence of $e$. This is the new answer for "How do I get there?".
