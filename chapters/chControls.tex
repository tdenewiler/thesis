...
*** Include a table showing the effects of modifying the different gain values on rise time, overshoot, settling time, etc. to show how coupled all of those terms are when trying to find stable values and contrast that with the gains for the Lyapunov controller as in \S\ref{sec:trajectoryCurvature}. ***
