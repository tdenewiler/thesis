...
% Simulate the physics for the trajectory to get the
% state variables through time. The state variables
% are y(1) = e, y(2) = alpha and y(3) = theta. This
% is basically planner output.
[t, y] = ode45(@f3, [0 10], [e a th]);

% Set up vectors to hold x and y position, heading and
% velocity of robot.
xpos = zeros(length(t));
ypos = zeros(length(t));
w = zeros(length(t));
...
% At each time step calculate the forward velocity and
% angular rate commands to issue to the robot.
for i = 1:length(t)
